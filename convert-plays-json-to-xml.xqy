...
declare function local:convert-play($map as element()) as element()
{
  element play {
    ...
    if ($map/xpf:string[@key="updated_at"]) then attribute updated {$map/xpf:boolean[@key="updated_at"]/text()} else (),
    if ($map/xpf:string[@key="expert_validated"]) then attribute validated {$map/xpf:boolean[@key="expert_validated"]/text()} else (),
    ...
    if ($map/xpf:string[@key="_packed_id"]) then attribute packed {$map/xpf:number[@key="_packed_id"]/text()} else (),
    ...
    if ($map/xpf:number[@key="acts"]/text()) then element acts {$map/xpf:string[@key="acts"]/text()} else (),
    if ($map/xpf:string[@key="prose_vers"]/text()) then element style {$map/xpf:string[@key="prose_vers"]/text()} else (),
    if ($map/xpf:boolean[@key="prologue"]/text()) then element  prologue {$map/xpf:string[@key="prologue"]/text()} else ()
  }
